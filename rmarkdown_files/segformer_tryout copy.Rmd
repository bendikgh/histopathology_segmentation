...
image_processor = SegformerImageProcessor(do_resize=False) # return_tensors="pt") # return_tensors="pt")
# jitter = ColorJitter(brightness=0.25, contrast=0.25, saturation=0.25, hue=0.1)

train_dataset = SegformerDataset(train_image_files, train_seg_files, transform=image_processor.preprocess)
...
```{python}
plt.imshow(probabilities[0].permute(1, 2, 0).detach().cpu().numpy())
plt.show()
plt.imshow(train_dataset.get_image(0).permute(1, 2, 0).to(torch.uint8))
plt.show()
```
...
for epoch in range(num_epochs):
    ...
    for images, labels in train_loader:
        ...
        # Compute loss
        # For CrossEntropyLoss, labels need to be of type Long and might need to be squeezed if they contain an extra dimension
