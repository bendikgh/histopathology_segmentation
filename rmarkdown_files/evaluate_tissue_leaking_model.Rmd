...
def calculate_f1_score(
    ...
    for batch_idx, (image_batch, mask_batch) in enumerate(dataloader):
        ...
        for idx in range(output_batch.shape[0]):
            ...
            # ax.autoscale(False)
            # ax.plot(peak_points_pred[:, 1], peak_points_pred[:, 0], 'o', color='black', markersize=0.5)
            # ax.axis('off')
            ...
            for y, x in peak_points_pred:
                ...
                TP_tmp = TP
                for cell in cell_annotation_list[
                    cell_annotation_list[:, 2] != cell_type
                ]:
            ...
            num_cells = len(cell_annotation_list)
            FN = num_cells - (TP + FP)
            if FN < 0:
                FN = 0
            dice_score = (2.0 * TP) / (2*TP + FP + FN)
...
# Dice score on test set: 0.7220074534416199
# Dice score on validation set: 0.6804498434066772
```
