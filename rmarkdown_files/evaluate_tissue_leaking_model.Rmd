...
def make_prediction_map(outputs):
    ...
    for output in outputs:
        ...
        all_cells = peak_local_max(cells, min_distance=10, labels=np.logical_or(mask==1, mask==2)) #np.logical_or(mask==1, mask==2)
...
def translate_green_blue_to_different_colors(tensor, color_for_green, color_for_blue):
    """
    Translates green and blue pixels in the image tensor to two specified different colors.

    Parameters:
    tensor (numpy.ndarray): Image tensor with shape (channels, height, width).
    color_for_green (list or tuple): The new color to replace green pixels, represented as [R, G, B].
    color_for_blue (list or tuple): The new color to replace blue pixels, represented as [R, G, B].
    ...
    for i in range(tensor.shape[1]):
        for j in range(tensor.shape[2]):
            ...
            if torch.all(tensor[:, i, j] == torch.tensor([0, 1, 0])):
                modified_tensor[:, i, j] = color_for_green
            # Check if the pixel is blue
            elif torch.all(tensor[:, i, j] == torch.tensor([0, 0, 1])):
                modified_tensor[:, i, j] = color_for_blue
...
def calculate_dice_score(dataloader, model, device): 
    ...
    for (images, masks) in dataloader: 
        ...
        mask = translate_green_blue_to_different_colors(masks[0].squeeze(), torch.tensor([1, 1, 0]), torch.tensor([1, 0, 1]))
        ...
        # Adjust the subplot parameters to minimize white space
...
def calculate_dice_score(dataloader, model, device):
    model.eval()
    total_dice_scores = []
    pixel_radius = int(1.4 / 0.2)
    search_radius = 8*pixel_radius
    ...
    for (images, masks) in dataloader:
        ...
        for i in range(outputs_cpu.shape[0]):
            ...
            for pred_point in peak_points_pred:
                ...
                correct_classified_points_tmp = correct_classified_points
                for dx in range(-search_radius, search_radius + 1):
                    if correct_classified_points > correct_classified_points_tmp:
                        break
            ...
            if num_gt_cells + num_pred_cells > 0:
                dice_score = (2.0 * correct_classified_points) / (num_gt_cells + num_pred_cells)
...
model.to(device) 
model.load_state_dict(torch.load("outputs/models/2024-01-03_15-47-23_deeplabv3plus_tissue_leaking_lr-0.0001_dropout-0.5_backbone-resnet50_epochs-110.pth"))
model.eval()
